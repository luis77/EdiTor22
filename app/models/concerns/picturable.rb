module Picturable
	...
	if self.respond_to?(:imagen)
  	...
  	def change!(image) #en este metodo vamos a recibir el id de la persona a la que vamos a seguir. el signo de admiracion se usa como convencion cuando se va a crear o cambiar algo
    	attachment.create!(imagen: image) #el id del amigo al que vamos a seguir, es el que recibiremos como parametro
  	end
	  ...
	  def archivo=(archivo) #creamos un metodo archivo
	  	unless archivo.blank? #validamos que el archivo no venga en blanco
	  		...
	  		if self.respond_to?(:imagen)
	  			self.imagen = archivo.original_filename #guarda el nombre original del archivo  		
	  		end
	  		self.formato = archivo.original_filename.split(".").last.downcase #guarda la extension del archivo(formato). lo guardamos partiendo la cadena del nombre basado en el punto . , tomando la ultima parte de esa cadena y pasandola a miniscula
	  end
	end

	def path_archivo
		File.join PATH_ARCHIVOS, "#{self.id}.#{self.formato}"   #devuelve la ruta especifica de ese archivo. coloca el nombre y la extension cuando cuarda la imagen. ej: para el attachment id 1 la imagen se llamara 1 y la extension
	end
